MaterialCalculator - 工业材料合成统计工具
MaterialCalculator 是一款基于 WPF (C#) 前端界面与 Python 计算引擎相结合的桌面工具。它专门用于处理复杂的多层级工业合成表（BOM），通过 Excel 维护配方，并能自动计算材料缺口、生成合成树以及按层级规划生产任务。
